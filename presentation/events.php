...
$userData = $eventData->getUserById($userId);

$skills = $eventData->getAllSkills();
$coordinators = $eventData->getAllCoordinators();
$organizers = $eventData->getAllOrganizers();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if ($_POST['action'] === 'create') {
        ...
        if ($result['success']) {
            ...
            if (isset($result['warning'])) $error = $result['warning'];
        } else {
    ...
    elseif ($_POST['action'] === 'cancel') {
        ...
        $result = $eventLogic->handleCancelEvent($_POST['eventId'], $reason);
        if ($result['success']) {
...
if ($action === 'cancel' && $eventId) {
    $cancelEvent = $eventData->getEventById($eventId);
    if (!$cancelEvent || !$eventData->canUserCancelEvent($eventId, $userId)) {
        header('Location: events.php');
        exit();
    }
}
...
<nav class="navbar navbar-expand-lg navbar-dark bg-dark">
    ...
    <div class="container mt-4">
                    ...
                    <div class="col-md-6 mb-4">
                        <div class="card h-100">
                            ...
                            <div class="card-body">
                                ...
                                <?php elseif ($eventLogic->isEventOver($event)): ?>
                                ...
                                <p><strong>Organizer:</strong> <?php echo htmlspecialchars($event['organizerName']); ?></p>
        ...
        <?php elseif ($action === 'cancel' && $cancelEvent): ?>
            <h2>Cancel Event: <?php echo htmlspecialchars($cancelEvent['eventName']); ?></h2>
            
            <div class="card mb-4">
                <div class="card-body">
                    <div class="alert alert-warning">
                        <i class="ri-alert-line"></i> 
                        <strong>Warning:</strong> Cancelling this event will notify all registered volunteers and coordinators via email.
                    </div>
                    
                    <p><strong>Event:</strong> <?php echo htmlspecialchars($cancelEvent['eventName']); ?></p>
                    <p><strong>Organizer:</strong> <?php echo htmlspecialchars($cancelEvent['organizerName']); ?></p>
                    <p><strong>Date:</strong> <?php echo date('M d, Y', strtotime($cancelEvent['startDate'])); ?> 
                       to <?php echo date('M d, Y', strtotime($cancelEvent['endDate'])); ?></p>
                    <p><strong>Location:</strong> <?php echo htmlspecialchars($cancelEvent['location']); ?></p>
                </div>
            </div>
            ...
            <form method="POST">
                ...
                <div class="mb-3">
                    ...
                    <textarea name="cancel_reason" class="form-control" rows="4" 
                              placeholder="Provide a reason for cancelling this event..."></textarea>
                </div>
                
                <button type="submit" class="btn btn-danger" 
                        onclick="return confirm('Are you sure you want to cancel this event? This action cannot be undone.')">
                    <i class="ri-close-circle-line"></i> Cancel Event
                </button>
            ...
            <form method="POST" enctype="multipart/form-data" class="row g-3">
                ...
                <?php if ($userRole === 'Organizer'): ?>
                    <div class="col-md-6">
                        ...
                        <input type="text" class="form-control" value="<?php echo htmlspecialchars($userData['name']); ?>" readonly>
                        <small class="text-muted">You are the organizer</small>
                    </div>
                <?php elseif ($userRole === 'Admin'): ?>
                    <div class="col-md-6">
                        <label class="form-label">Select Organizer</label>
                        <select name="createdBy" class="form-select">
                            <option value="<?php echo $userId; ?>">Myself (Admin)</option>
                            <?php foreach ($organizers as $organizer): ?>
                                <option value="<?php echo $organizer['userId']; ?>"
                                    <?php echo (isset($editEvent['createdBy']) && $editEvent['createdBy'] == $organizer['userId']) ? 'selected' : ''; ?>>
                                    <?php echo htmlspecialchars($organizer['name'] . ' (' . $organizer['email'] . ')'); ?>
                                </option>
                            <?php endforeach; ?>
                        </select>
                    </div>
                ...
                <div class="col-md-6">
                    ...
                    <select name="category" class="form-select" id="categorySelect">
                        ...
                        <?php 
                        $allCats = ['Charity', 'Education', 'Environment', 'Health', 'Community', 
        ...
        <?php elseif ($action === 'assign' && $assignEvent): ?>
            <h2>Assign Coordinators: <?php echo htmlspecialchars($assignEvent['eventName']); ?></h2>
            
            <div class="card mb-4">
                <div class="card-body">
                    <p><strong>Organizer:</strong> <?php echo htmlspecialchars($assignEvent['organizerName']); ?></p>
                    <p><strong>Date:</strong> <?php echo date('M d, Y', strtotime($assignEvent['startDate'])); ?></p>
                </div>
            </div>
        ...
        <?php else: ?>
            <div class="d-flex justify-content-between align-items-center mb-4">
                <h2>Events Management 
                    <?php if ($userRole === 'Organizer'): ?>
                        <small class="text-muted">(My Events)</small>
                    <?php endif; ?>
                </h2>
                <div>
                    <?php if (in_array($userRole, ['Admin', 'Organizer'])): ?>
                        <a href="events.php?action=create" class="btn btn-success me-2">+ Create Event</a>
                    <?php endif; ?>

                    <?php if (in_array($userRole, ['Admin', 'Organizer'])): ?>
                        <a href="export_csv.php?type=events_filtered" class="btn btn-info me-2" onclick="showCsvLoading(this)">Export Data as CSV</a>
                    <?php endif; ?>

                    <?php if ($userRole === 'Coordinator'): ?>
                        <a href="events.php?dashboard=1" class="btn btn-info">My Events</a>
                    <?php endif; ?>
                </div>
            </div>
            ...
            <form method="GET" class="row g-3 mb-4 p-3 border rounded bg-light">
                ...
                <div class="col-md-2">
                    ...
                    <a href="events.php" class="btn btn-secondary"><i class="ri-refresh-line"></i></a>
                </div>
            </form>
                ...
                <table class="table table-striped table-hover">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td><?php echo htmlspecialchars($event['organizerName'] ?? 'Unknown'); ?></td>
                            ...
                            <td>
                                <div class="d-flex gap-1 flex-wrap">
                                    <a href="view_event.php?id=<?php echo $event['eventId']; ?>" 
                                       class="btn btn-sm btn-info"><i class="ri-eye-line"></i></a>

                                    <a href="view_volunteers.php?eventId=<?php echo $event['eventId']; ?>" 
                                       class="btn btn-sm btn-outline-danger"><i class="ri-user-line"></i></a>
                                    
                                    <?php if ($eventData->canUserEditEvent($event['eventId'], $userId) && $event['status'] != 'Cancelled'): ?>
                                        <a href="events.php?action=edit&id=<?php echo $event['eventId']; ?>" 
                                           class="btn btn-sm btn-success"><i class="ri-pencil-line"></i></a>
                                    <?php endif; ?>
                                    
                                    <?php if ($eventData->canUserCancelEvent($event['eventId'], $userId) && $event['status'] != 'Cancelled' && !$eventLogic->isEventOver($event)): ?>
                                    ...
                                    <?php if ($userRole === 'Admin'): ?>
                                        <a href="events.php?action=assign&id=<?php echo $event['eventId']; ?>" 
                                           class="btn btn-sm btn-primary"><i class="ri-user-add-line"></i></a>
